...
if [[ $VER == "" ]]; then
  # 获取最新版本
  echo "获取最新版..."
  latest_version=`curl -s 'http://auth.doudun.net/master/upgrades?latest=1' | grep -Po 'v\d+\.\d+.\d+' || true`
...
cd /opt/
download "https://gitee.com/edr4869/cdnfly/raw/main/master/$tar_gz_name" "https://gitee.com/edr4869/cdnfly/raw/main/master/$tar_gz_name" "$tar_gz_name"
...
if [ -f /opt/cdnfly/master/view/upgrade.so ]; then
	wget https://gitee.com/edr4869/cdnfly/raw/main/cdnfly/api.py -O /opt/venv/lib/python2.7/site-packages/requests/api.py
